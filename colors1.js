...
function nightDayHandler(self) { //nightDayHandler 함수에 this 라는 값이 대입됨.
  var target = document.querySelector('body'); //변수 target 이라는 변수는 자바스크립트가  "querySelector 라는 메쏘드의 인자로써 body 태그를 넣었을때 불러와지는 데이터" 라는 뜻-- >
  if (self.value === 'colors1 객체&메쏘드 night mode on') {
    //<!--nightDayHandler 라는 함수에 this가 들어가 있고, this의 value 값과 원래 상태의 밸류값의 비교 연산을 실시해서, 참일경우와 거짓일경우에 따라서 프로그램을 실행시킨다-- >
    Body.setBackgroundColor('black'); // BodySetBackgroundColor 라는 함수의 파라미터에다가 orange 라는 인수값을 넣는다.
    Body.setFontColor('yellow'); // BodySetFontColor 라는 함수의 파라미터에다가 red 라는 인수값을 넣는다.
    self.value = 'colors1 객체&메쏘드 day mode on'; //< !--if 값이 참 일경우, 현재 소속된 태그의 밸류값을 'day mode on' 으로 치환한다-- >
    ...
    self.value = 'colors1 객체&메쏘드 night mode on'; // nightDayHandler 라는 함수의 파라미터값인 self 에다가 인수 this 를 넣는다.
